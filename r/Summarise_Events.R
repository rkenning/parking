...
#get a map based 
map <- get_map(location = "Bath, Uk", zoom = 12)

#Draw map points for both the Event and Parking locations
mappoints <- ggmap(map) + geom_point(aes(x=long, y=lat),data=dfmap, colour="Blue" ,size=3) + geom_point(aes(x=as.numeric(dfgroupsub$Long), y=as.numeric(dfgroupsub$Lat)),data=dfgroupsub, colour="Red" ,size=2) 
